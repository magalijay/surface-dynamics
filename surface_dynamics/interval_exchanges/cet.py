r"""
Circle Exchange Transformations and Linear Involution
...
class CircleExchangeTransformation:
    r"""
    ...
    - ``rotation`` - the angle of rotation between the top permutation and the bottom permutation 
    ...
    def __init__(self, permutation=None, lengths=None, rotation=None, base_ring=None):
        ...
        - ``rotation`` - the list of two elements... TODO 
        ...
        TESTS::
            ...
            sage: p = iet.CET(('a b c','c b a'),{'a':1,'b':1,'c':1},[0,3/2])
            sage: p.permutation()
            a b c
            a c b
            sage: p.lengths()
        ...
        if permutation is None or lengths is None or rotation is None: #Maybe rotation is 0,0 if None ?
            ...
            if self._lengths is lengths: # question 
                # NOTE: vector(.) never performs copy
                self._lengths = self._lengths.__copy__()
            l = [0] * (len(permutation)+2)
            l[:-1] = self._lengths.__copy__()
            l[-2:] = rotation # note : pour avoir les longueurs et la rotation
            l = vector(l)
            #V = l.parent()
           ...
           # print(self._lengths)
            for level in range(2):
                while rotation[level] < self._base_ring.zero() :
                    rotation[level] += length
           # print(rotation)
                while rotation[level] >= length :
                    rotation[level] -= length
            # initialize the permutation so that the first top (resp. bottom) label corresponds to the first complete interval after 0 
                k = -1
            #print(self._permutation._labels)
                i = self._permutation._labels[level][k] #j
            #print(self._permutation._alphabet)
            #i = self._permutation._alphabet.rank(j)
                x = self._lengths[i]
                c = 0
                while x < rotation[level] :
                    c += 1                
                    k -= 1
                    i = self._permutation._labels[level][k]
                #i = self._permutation._alphabet.rank(j)
                    x += self._lengths[i]
                if c != 0 :
                    x -= self._lengths[i]
                    ...
                    for i in range(n-c,n) :
                        labels_level.append(permutation._labels[level][i])
                    for i in range(n-c) :
                        labels_level.append(permutation._labels[level][i]) 
                    self._permutation._labels[level] = labels_level            
    ...
    def permutation(self): # question : un moyen pour utiliser les fonctions iet pour des cet (permutation, length, lengths, translations, (half)flipped-translations) ?
        r"""
        Returns the permutation associated to this iet. Note that it is not uniquely determined, it depends on the rotation associated to the iet.
    ...
    def rotation(self):
        r"""
        Returns the rotation associated to this iet. Note that it is not uniquely determined, it depends on the permutation associated to the iet.
    ...
    def translations(self):
        ...
        if self._permutation._flips is not None:
            raise TypeError("the cet has flip(s), not implemented yet")# the function flipped_translations must be used instead")
    ...
    def is_identity(self):
        ...
        EXAMPLES::
            ...
            True
        """
        return self._permutation.is_identity() and (self._rotation[0]==self._rotation[1]) # (self.rotation() == 0) # TODO : vérifier le cas d'une permutation cyclique avec la rotation qui compense
    ...
    def inverse(self):
        ...
        We can verify with the method .is_identity()::
            ...
            sage: s = iet.CET(p, [1, sqrt(2), sqrt(3), sqrt(5)], [0.2, sqrt(7)])
...
# TODO 1st ! issue : rotation is now a list 
        ...
        while i_other < n_other and i_self < n_self:
            j_other = interval_other[i_other] # TODO : verif flip
        ...
        while i_other < n_other and i_self <= i_self_start:
          ...
          #  print(i_self,i_other)
            ...
            if i_self == n_self - 1 : # which case gives True here ? one interval or ... ?
                self_next_sg = self_sg[0] + total_length
            else:
    ...
    def __ne__(self, other):
        ...
        return (
            ...
            self._lengths != other._lengths) # question : on devrait en plus dire qu'il y a égalité quand les perm et rotation sont différentes mais donnent la même chose en terme de fonction - devrait marcher, à vérifier
    ...
    def singularities(self):
        ...
        EXAMPLES::
            ...
            [[0, 1/2], [1/4, 7/4]]
        """
# Attention, on a supprimé la dernière singularité, qui correspond à la première !
...
# TODO
    ...
    def plot_function(self,**d):
        ...
        function. Transform a circle of perimeter x into
